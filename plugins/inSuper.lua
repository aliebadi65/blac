...
--vardump(result)
local text = "کاربر از سوپر گروه اخراج شد "..cb_extra.receiver.."\n\n"
local i = 1
for k,v in pairsByKeys(result) do
if not v.print_name then
	name = " "
...
local function unlock_group_links(msg, data, target)
  ...
  if group_link_lock == 'no' then
    return 'د تبلیغ از قبل غیرفعال بوده است'
  else
...
function get_message_callback(extra, success, result)
	...
	elseif get_cmd == 'mute_user' then
		...
		if is_muted_user(chat_id, user_id) then
			...
			send_large_msg(receiver, "["..user_id.."] حذف شد از لیست کاربر بیصدا")
		elseif is_admin1(msg) then
			mute_user(chat_id, user_id)
			send_large_msg(receiver, " ["..user_id.."] اضافه شده به لیست کاربران بیصدا")
		end
	end
end
